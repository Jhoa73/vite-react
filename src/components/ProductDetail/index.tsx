...
const ProductDetail: FC = () => {
	const { productPreview: product, setProductPreview } =
		useContext(ShoppingCartContext);

	if (!product) {
		return <h2>Sin productos</h2>;
	}
		...
		<aside className="product-detail flex flex-col fixed right-0 border border-black rounded-lg bg-white p-6">
			...
			<div className="mt-3">
				<figure className={"relative px-6"}>
					<span className="absolute bottom-0 right-0 bg-black/60 rounded-lg text-white text-xs m-2 px-3 py-0.5">
						{product.category}
					</span>
					<img
						src={product.image}
